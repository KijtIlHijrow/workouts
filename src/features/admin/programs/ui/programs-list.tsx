    ...
    return (
      <div className="card bg-base-100 shadow-xl">
        <div className="card-body flex flex-col items-center justify-center py-12">
          ...
          <p className="text-base-content/60 text-center max-w-md">Start by creating your first workout program.</p>
        </div>
      </div>
    );
  }
            ...
            <tr key={program.id}>
              ...
              <td>
                <div className="text-sm">
                  <div className="font-semibold">{program.durationWeeks} weeks</div>
                  <div className="text-xs opacity-50">{program.sessionsPerWeek} sessions/wk</div>
                </div>
              </td>
              <td>
                <div className="text-sm">
                  <div>
                    {program.totalWeeks} wks • {program.totalSessions} sessions
                  </div>
                  <div className="text-xs opacity-50">{program.totalExercises} exercises</div>
                </div>
              </td>
              <td>
              ...
              <td>
                <div className="flex gap-1">
                  <Link className="btn btn-ghost btn-xs" href={`/programs/${program.slug}`} target="_blank" title="View program">
                    <Eye className="h-4 w-4" />
                  </Link>
                  <Link className="btn btn-ghost btn-xs" href={`/admin/programs/${program.id}/edit`} title="Manage program">
