# Self-Hosting Workouts

Deploy **Workouts** on your own server using Docker. This guide provides two deployment options with step-by-step instructions.
...
**Prefer watching?** Watch our [3-minute video guide on self-hosting Workouts](https://www.youtube.com/watch?v=HQecjb0CfAo):

<p align="center">
  <a href="https://www.youtube.com/watch?v=HQecjb0CfAo">
    <img src="https://img.youtube.com/vi/HQecjb0CfAo/maxresdefault.jpg" alt="Self-Host Workouts in 3 Minutes Video Guide" style="max-width: 400px;">
  </a>
</p>
...
mkdir -p ~/apps
cd ~/apps
git clone https://github.com/Snouzy/workouts.git
cd workouts
```
...
DATABASE_URL=postgresql://username:password@postgres:5432/workouts
```
...
DATABASE_URL=postgresql://username:password@your-db-host:5432/workouts
```
...
docker run -d --name workouts -p 3000:3000 --env-file .env workouts
```
...
2. **Create Nginx config:**
    ...
    ```bash
    sudo nano /etc/nginx/sites-available/workouts
    ```
...
3.  **Enable and reload:**
    ```bash
    sudo ln -s /etc/nginx/sites-available/workouts /etc/nginx/sites-enabled/
...
docker start workouts
docker stop workouts

# View logs
docker logs -f workouts
...
```bash
# Check logs
docker compose logs workouts  # or docker logs workouts

# Verify environment variables using docker compose
docker compose exec workouts env | grep DATABASE_URL  # or docker exec workouts env | grep DATABASE_URL
```

#### Database Connection Issues
```bash
# Test database connectivity (Docker Compose)
docker compose exec postgres psql -U postgres -d workouts -c "SELECT 1;"
...
4. **Search existing issues** or create a new one on [GitHub](https://github.com/Snouzy/workouts/issues)
...
### Community & Support
- **GitHub**: [Repository](https://github.com/Snouzy/workouts) | [Issues](https://github.com/Snouzy/workouts/issues)
